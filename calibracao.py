...
# ==================== CONFIGURAÇÃO GLOBAL ====================
st.set_page_config(page_title="Controle de Solo - Metrosul & Corsan", page_icon="🧪", layout="centered")
...
# ==================== NAVEGAÇÃO (MENU) ====================
st.sidebar.title("🛠️ Selecione o Ensaio")
opcao = st.sidebar.radio("Ir para:", ["Densidade In Situ", "Ensaio DCP"])
st.sidebar.divider()

# ==================== MÓDULO 1: DENSIDADE IN SITU ====================
if opcao == "Densidade In Situ":
    st.title("🧪 Painel de Controle - Metrosul")
    
    def gerar_pdf_ensaio_densidade(d):
        pdf = FPDF()
        pdf.add_page()
    ...
    if gc > 0:
        ...
        st.download_button("📥 Baixar PDF Metrosul", gerar_pdf_ensaio_densidade(dados_pdf), "Relatorio_Metrosul.pdf", "application/pdf", use_container_width=True)

# ==================== MÓDULO 2: ENSAIO DCP ====================
elif opcao == "Ensaio DCP":
    st.markdown('<h1 style="color: #0F4C81;">🏗️ Ensaio de Compactação DCP - Corsan</h1>', unsafe_allow_html=True)
    ...
    def gerar_pdf_dcp(dados):
        ...
        pdf.cell(190, 12, "RELATORIO DE ENSAIO DCP", ln=True, align='C')
        pdf.set_font("Arial", "", 10)
        pdf.set_text_color(0, 0, 0)
        pdf.cell(190, 6, f"Gerado em: {datetime.now().strftime('%d/%m/%Y')}", ln=True, align='C')
        pdf.ln(5)
        ...
        pdf.cell(55, 10, "Penetracao (mm)", border=1, align='C')
        pdf.cell(90, 10, "IPD Parcial", border=1, align='C', ln=True)
        ...
        pdf.ln(5)
        pdf.set_font("Arial", "B", 12)
        pdf.cell(190, 10, f"STATUS FINAL: {dados['status']}", border=1, ln=True, align='C')
    ...
    marco_zero = parse_float(st.text_input("Penetração inicial (Marco Zero - mm)", value="0.0"))

    for idx, leitura in enumerate(st.session_state.leituras):
        c_v, c_d = st.columns([5, 1])
        st.session_state.leituras[idx] = parse_float(c_v.text_input(f"Leitura {(idx+1)*3} golpes (mm)", value=f"{leitura:.1f}", key=f"dcp_{idx}"))
        if c_d.button("🗑️", key=f"del_dcp_{idx}"):
    ...
    if st.session_state.leituras:
        ipd_final = (st.session_state.leituras[-1] - marco_zero) / (len(st.session_state.leituras) * 3)
        ...
        if os_id and endereco:
            d_pdf = {"os": os_id, "endereco": endereco, "material": st.session_state.material,
                     "marco_zero": marco_zero, "leituras": st.session_state.leituras,
                     "ipd_final": ipd_final, "status": status}
            st.download_button("📄 Baixar PDF DCP", gerar_pdf_dcp(d_pdf), "DCP_Relatorio.pdf", "application/pdf", use_container_width=True)
